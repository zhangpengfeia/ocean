...
1. 请求接口获取用户信息。
2. 将返回数据渲染到用户栏并处理样式，如果用户信息不存在使用默认信息。
...
+ 需求
  1. 新建目录myinfo
  2. 在目录中新建页面info-ba sic.html，新建js
...
+ 问题
  ...
  原因：该插件底层用到了canvas技术，canvas加载图片不允许协议跨域，所以在本地会报错，当上传到服务器时不会报错
