...
+ **3.finally()**
  ...
  ```js
  不管状态如何都会执行的操作
  p.finally() 不接受任何参数，返回原来的值
  ```

+ **all()**

  ```js
  将多个 Promise 实例，包装成一个新的 Promise 实例，所有的都成功 Promise 的状态才会为 fulfillted,否则为 rejected
  
  Promise.all([p1,p2,p3]) 参数是数组，返回的也是一个数组，存的是每个 Promise 实例返回的结果。
  ```
